...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    // Get supplier information if exists (same as GET method)
    if (data.supplier_id) {
      try {
        const { data: supplier } = await supabase
          .from('user_parties')
        ...
        console.warn('Could not fetch supplier for updated purchase:', id);
      }
    }
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    // Delete the purchase directly - we'll skip balance updates for simplicity
