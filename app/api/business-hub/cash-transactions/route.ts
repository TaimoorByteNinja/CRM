...
export async function GET(req: NextRequest) {
  console.log('GET /api/business-hub/cash-transactions called');
  try {
    ...
    // Log each transaction to see the structure
    if (data && data.length > 0) {
      console.log('Sample cash transaction data structure:', JSON.stringify(data[0], null, 2));
    }
...
export async function POST(request: NextRequest) {
  console.log('POST /api/business-hub/cash-transactions called');
  try {
    ...
    const transactionId = body.transaction_id || `cash-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
    ...
    console.log('Successfully created cash transaction:', data);
    return NextResponse.json(data);
  } catch (error) {
